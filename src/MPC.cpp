...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += 1 * CppAD::exp(CppAD::pow(0.7 * vars[cte_start + i], 2)) - 1;
      fg[0] += 10 * CppAD::pow(vars[epsi_start + i], 2);
      fg[0] += 0.25 * CppAD::pow(vars[v_start + i] - ref_v, 2);
    }
    ...
    for (int i = 0; i < N - 1; i++){
      fg[0] += (0.17 * CppAD::pow(CppAD::pow(vars[v_start+i],2)/Lf*vars[delta_start+i], 2)  // Lateral Acceleration
                  + CppAD::pow(vars[v_start + i + 1] - vars[v_start + i], 2));       // Longitudinal Acceleration
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  Dvector vars_upperbound(n_vars);  // Set lower and upper limits for variables.
